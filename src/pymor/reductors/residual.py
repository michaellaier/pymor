...
def reduce_instationary_residual(operator, mass, dt, functional=None, RB=None, product=None, extends=None):
    """Reduced basis residual reductor with mass operator.
    ...
    logger = getLogger('pymor.reductors.residual.reduce_instationary_residual')
    ...
    try:
        ...
        return (NonProjectedInstationaryResidualOperator(operator, mass, functional, dt, product),
                NonProjectedReconstructor(product),
                {})
    ...
    return (InstationaryResidualOperator(operator, mass, functional, dt),
            GenericRBReconstructor(residual_range),
            {'residual_range_dims': residual_range_dims})


class InstationaryResidualOperator(OperatorBase):
    """Returned by :func:`reduce_instationary_residual`."""
    ...
    def apply(self, U, ind, mu=None):
        V = self.operator.apply(U, ind=ind, mu=mu)
        V.axpy(1./self.dt, self.mass.apply(U, ind=ind, mu=mu))
        V.axpy(-1./self.dt, self.mass.apply(U, ind=ind-1, mu=mu))
        if self.functional:
            F = self.functional_vector or self.functional.as_vector(mu)
            V.axpy(-1., F)
        return V
    ...
    def projected_to_subbasis(self, dim_range=None, dim_source=None, name=None):
        return InstationaryResidualOperator(self.operator.projected_to_subbasis(dim_range, dim_source),
...
class NonProjectedInstationaryResidualOperator(InstationaryResidualOperator):
    """Returned by :func:`reduce_instationary_residual`.
    ...
    def __init__(self, operator, mass, functional, dt, product):
        super(NonProjectedResidualOperator, self).__init__(operator, mass, functional, dt)
        self.product = product

    def apply(self, U, ind=None, mu=None):
        R = super(NonProjectedInstationaryResidualOperator, self).apply(U, ind=ind, mu=mu)
        if self.product:
            R_riesz = self.product.apply_inverse(R)
            return R_riesz * (np.sqrt(R_riesz.dot(R)) / R_riesz.l2_norm())[0]
        else:
